...
- **Exploration:** when nothing's reachable within 64 blocks, instead of giving up it `explore()`s - walks ~48 blocks in a rotating compass direction (`GoalNearXZ`, 30s timeout) to load fresh chunks, bounded by `MAX_EXPLORE=12` dry wanders before truly failing. Turns "no reachable X" stalls into "keep looking."
...
- **Persistent collection:** `collectDrops` now waits for drops to settle (250ms) and sweeps up to 14 times.
