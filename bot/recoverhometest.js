...
{
  const d = provRecovery.homeRecoveryDecision({ hut, pos: worldSpawn, dist: 64 })
...
eq(provRecovery.homeRecoveryDecision({ hut, bed, resumeAt, pos: worldSpawn, dist: 64 }).source, 'hut', 'hut wins when all three known')
eq(provRecovery.homeRecoveryDecision({ bed, resumeAt, pos: worldSpawn, dist: 64 }).source, 'bed', 'bed wins when no hut')
eq(provRecovery.homeRecoveryDecision({ resumeAt, pos: worldSpawn, dist: 64 }).source, 'resume', 'build site is the last-resort anchor')
eq(provRecovery.homeRecoveryDecision({ resumeAt, pos: worldSpawn, dist: 64 }).anchor, resumeAt, 'resume anchor is the raw build site (no +2 offset)')
eq(provRecovery.homeRecoveryDecision({ bed, pos: worldSpawn, dist: 64 }).anchor, bed, 'bed anchor is the raw bed (no +2 offset)')

// ---- NOT far: already home (respawned AT the hut bed) -> do nothing -------------------
{
  const d = provRecovery.homeRecoveryDecision({ hut, pos: { x: 416, y: 66, z: 88 }, dist: 64 })
...
eq(provRecovery.homeRecoveryDecision({ hut, pos: { x: 416 + 60, y: 65, z: 87 }, dist: 64 }).far, false, '60b out (<64) -> not far')
// Just outside the threshold ring -> far.
eq(provRecovery.homeRecoveryDecision({ hut, pos: { x: 416 + 70, y: 65, z: 87 }, dist: 64 }).far, true, '70b out (>64) -> far')

// ---- Y is IGNORED (respawn Y varies; "far" is horizontal) ---------------------------
eq(provRecovery.homeRecoveryDecision({ hut, pos: { x: 416, y: 5, z: 87 }, dist: 64 }).far, false, 'right above/below home but deep underground -> not far (XZ only)')

// ---- no anchor at all -> never "far" (nothing to trek to; honest no-op) ---------------
eq(provRecovery.homeRecoveryDecision({ pos: worldSpawn, dist: 64 }).far, false, 'no hut/bed/resume -> not far (no home to go to)')
eq(provRecovery.homeRecoveryDecision({ pos: worldSpawn, dist: 64 }).anchor, null, 'no anchor -> null')
// no position -> cannot decide -> not far.
eq(provRecovery.homeRecoveryDecision({ hut, dist: 64 }).far, false, 'no position -> not far')
...
// ---- threshold comes from RECOVER_HOME_DIST when dist not passed ---------------------
{
  ...
  eq(provRecovery.homeRecoveryDecision({ hut, pos: worldSpawn }).far, false, 'RECOVER_HOME_DIST=1000 -> 425b not far')
  process.env.RECOVER_HOME_DIST = '32'
  eq(provRecovery.homeRecoveryDecision({ hut, pos: worldSpawn }).far, true, 'RECOVER_HOME_DIST=32 -> 425b far')
  delete process.env.RECOVER_HOME_DIST
  eq(provRecovery.homeRecoveryDecision({ hut, pos: worldSpawn }).far, true, 'default 64 -> 425b far')
