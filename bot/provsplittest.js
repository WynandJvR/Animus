...
// The split is only safe while that surface is unchanged AND not a stale copy.
{
  ...
  eq(pub.length, 185, 'facade: provision.js still exports exactly 185 public names') // -3: furnaceCountFor, ensureFurnaces, wildTerrainMovements OUT (DEAD-CODE AUDIT 2026-08-01 - a facade entry whose name appears in no other file, comments included, is not an API: nothing in the tree could reach these three, and an unreachable name on the public surface is exactly the drift this count exists to catch). +2 -1: HUNT_SOURCES + huntForDrop IN, gatherWool OUT (CAPABILITY REGISTRY S3 - the `hunt` producer kind and its ONE driver; the three hand-written mob-drop chases collapsed onto it, every mob drop declared in capabilities.js is now plannable by planProvision with no further wiring, and the hand-wired gatherWool one-off went with the acquireBed bolt-on that was its only caller); +2: ensureHomeShelter (#117 HOME_IS_A_NEED - the 'shelter' bootstrap producer) + sleepableNow (the pure world-condition the 'spawn' verdict gates on); +6: bedInPack, bedCandidates, acquireBed, placeBedNear, bedFootprint, bedWellPlaced (#107 SPAWN_BED); +2: secureBase, secureBaseGate (#67 SECURE_BASE); +2: ironGrindMinedReal, resetIronGrindMined (IRON_KEYSTONE); +1: deathSpotExclusion (#85 DEATH_SPOT_COST); +3: sealHomeDescents, sealDescentsGate, inBuildZone (#89 SEAL_HOME_DESCENTS); +2: worldTidy, litterSignature (#94 WORLD_TIDY); -2 +4: furnishHut + bedWellPlaced OUT, bedUsable + assertSpawnOn + ensureBedSite + upgradeBedPlacement IN (#110 ANCHOR_INVARIANT); +2: markHazardTraversal + setOperatorRouting (#112 HAZARD_NOT_LURE - the hazard release condition and the operator-routing latch); +1: escapeUpColumn (#116 ESCAPE_ACCOUNTABLE - the vertical rung for a submerged enclosure); +2: surveyWaterSite, chooseFarmSite (#118 FARM_SITED_FROM_HOME - the water-record qualifier and the home-anchored site comparison); +1: ensurePillarFiller (AUDIT 2026-07-29 FIX 12 - the pit-escape rung needs it; it was reachable only from the suicide-reset, so a blockless bot pillared 60x in its own shelter without moving); +2: gatherWool + woolCount (AUDIT FIX 16 - wool had NO entry in GATHER_SOURCES at all, so acquireBed could only try dye-chain recipes for wool it did not have and never made a bed); +1: digStaircaseUp (AUDIT FIX 17 - the pit rung can now DIG out when pillaring cannot lift it; the primitive existed but only climbToSurface could reach it, and that rung is gated on being underground); +1: noteFurnaceHoldings (#119b MY OWN FURNACE IS A PANTRY - provision-food must SETTLE a drained furnace debt through the SAME write-ahead that booked it; the alternative was a second copy of the three-slot read in the food chain, and two copies of a ledger rule is how the ledger drifts); +3: releaseMaintainLatch + releaseFoodLatch + releaseRecoveryLatches (2026-07-31 THE 4.5-HOUR GHOST - the five provision job latches are set true and cleared in a finally a HUNG await never reaches, so _maintaining alone reported 'maintenancePass' to activeJobInfo for four and a half hours while the scheduler dispatched nothing; stopMaintenance/stopSurvivalJob are COOPERATIVE and a hung await polls nothing, so the watchdog's terminal rung needs a FORCE release it can call)
