...
async function stampTest () {
  ...
  t('APPROACH: the doorstep cells are the body-sized column just OUTSIDE the door', () => {
    ...
    assert.strictEqual(cells.length, 4, 'doorstep feet+head, and the egress cell feet+head - a doorstep you cannot leave is not an approach')
    const out = H.outsideCell(SA, d)
    assert.deepStrictEqual(cells.slice(0, 2).map(c => c.x + ',' + c.y + ',' + c.z),
    ...
    // ...and the egress column runs straight OUT, never back toward the hut
    const away = { x: out.x - d.x, z: out.z - d.z }
    assert.deepStrictEqual({ x: cells[2].x, z: cells[2].z }, { x: out.x + away.x, z: out.z + away.z },
      'the way out must lead away from the hut - a doorstep you cannot leave is not an approach')
  })
  ...
  t('APPROACH: the LIVE geometry - it names exactly where the ghost door stood', () => {
    ...
    assert.deepStrictEqual(cells.map(c => c.x + ',' + c.y + ',' + c.z), ['190,68,-105', '190,69,-105', '190,68,-106', '190,69,-106'])
  })
